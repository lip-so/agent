...
class InstallerUI:
    ...
    def _create_port_finding_view(self):
        ...
        # A canvas-based button for consistent styling
    ...
    def _on_port_finder_button_enter(self, event):
        # Check if button is enabled by checking text color
    ...
    def _create_main_buttons(self):
        y, h, w = 530, 44, 180  # Increased width for icons
        total_width = w * 2
        ...
        for name, B in buttons.items():
            bg = self.canvas.create_rectangle(B['pos'], fill=self.colors['surface'], outline='', width=0) # Seamless
        ...
        # Add a divider
        divider_x = start_x + w
        self.canvas.create_line(divider_x, y + 10, divider_x, y + h - 10, fill=self.colors['border'])
            
        # Initially disable Find Ports button
    ...
    def _on_mouse_motion(self, event):
        for name, (x1, y1, x2, y2) in self.button_coords.items():
            ...
            # Only enable hover for buttons with primary text color
